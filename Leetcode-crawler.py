...
pickle_path = "/Users/ccrjohn8787/Documents/InterviewPrep/leetcode_pickle.pkl"
...
class Download(object):
    ...
    def get_parsed_html(self):
        if not hasattr(self, 'parsed_html') or not self.parsed_html:
            self.parsed_html = BeautifulSoup(self.get_html(), "html.parser")
...
class DownloadAll():
    ...
    def download_all(self):
        ...
        for tr in self.get_list():
            if len(tr.find_all('a')) > 0 and tr.find_all('a')[0].get('href') != '#':
                try:
                    url, rate = tr.find_all('a')[0].get('href'), tr.find_all('td')[3].text
                    problem_name = url.split('/')[2]
                    print "Adding problem: {}. Rate: {}. Count: {}".format(problem_name, rate, count)
                    res = self.download_class(url, rate).get_all()
                    self.problems_map[problem_name] = res
                    count+=1
                    if count >=330:
                        break
                except Exception, e:
                    print "Error processing problem {}, {}".format(tr, e)
        output = open(pickle_path, 'wb')
        pickle.dump(self.problems_map, output)
        print 'Donezo!'

    def get_problem(self, problem):
        if not self.problems_map:
            pkl_file = open(pickle_path, 'rb')
            self.problems_map = pickle.load(pkl_file)
        if problem in self.problems_map:
            return self.problems_map[problem]
        return None
